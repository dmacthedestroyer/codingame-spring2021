...
export function run() {
  ...
  while (true) {
    ...
    const turn: Turn = { day, nutrients, sun, score, oppSun, oppScore, oppIsWaiting, cells, trees };
    const actualSeedMoves = possibleMoves.filter(m => m.indexOf("SEED") >= 0);
    const calculatedSeedMoves = calculateMyMoves(turn)
      .filter(m => m._type === "seed")
      .map(moveString);
    if (actualSeedMoves.length !== calculatedSeedMoves.length) {
      printErr(turn);
      const [calculatedDiff, actualDiff] = differences(calculatedSeedMoves, actualSeedMoves);
      calculatedDiff.sort();
      actualDiff.sort();
      printErr({
        calculated: {
          count: calculatedSeedMoves.length,
          diff: calculatedDiff,
          all: calculatedSeedMoves
        },
        actual: {
          count: actualSeedMoves.length,
          diff: actualDiff,
          all: actualSeedMoves
        }
      });
...
function differences<T>(s1: T[], s2: T[]): [T[], T[]] {
  return [s1.filter(t1 => !s2.find(t2 => t1 === t2)), s2.filter(t2 => !s1.find(t1 => t2 === t1))];
}
